"""Phase D: train a walking PPO policy in MuJoCo, with domain randomization
and an imitation reward against the existing hand-coded trot.

See drafts/PLAN_SIM2REAL_RL_PIPELINE.md.

Usage:
    python train_walk.py --timesteps 2000000 --n-envs 8
"""
...
from env import DogzillaWalkEnv
...
def main():
    ...
    if args.check_only:
        check_env(DogzillaWalkEnv())
        print("Env check passed.")
        return

    vec_env = make_vec_env(lambda: DogzillaWalkEnv(), n_envs=args.n_envs, vec_env_cls=SubprocVecEnv)
    ...
    out_path = os.path.join(CHECKPOINT_DIR, "ppo_walk")
    ...
    print("Next: python export_onnx.py --checkpoint checkpoints/ppo_walk.zip --out checkpoints/policy_walk.onnx")
