...
async function main() {
  const uaktAmount = process.argv[2] || "20000000"; // default 20 AKT
  ...
  try {
    ...
    const result = execFileSync("wsl.exe", [
      "-d", "Ubuntu-24.04", "--", "akash", "tx", "bme", "mint-act", `${uaktAmount}uakt`,
      "--node", "https://akash-rpc.polkachu.com:443",
      "--chain-id", "akashnet-2",
      "--from", "junoclaw-autodeploy",
    ...
    console.log(`Minted ACT from ${uaktAmount} uakt:`, JSON.parse(result).txhash);
  } finally {
