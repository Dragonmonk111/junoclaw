      ...
      <div className="flex-1 px-5 py-4 space-y-4">
        ...
        {config && (
          <div className="flex items-center gap-4 text-[11px] px-3 py-2 rounded-lg" style={{ background: 'rgba(255,255,255,0.02)' }}>
            ...
            <span style={{ color: '#6b6a8a' }}>Rewards: <span style={{ color: config.rewardMode === 'stake_times_accuracy' ? '#00d4aa' : config.rewardMode === 'stake_weighted' ? '#7c6bff' : '#c0bfd6' }}>{config.rewardMode === 'stake_times_accuracy' ? 'stake×accuracy' : config.rewardMode === 'stake_weighted' ? 'stake-weighted' : 'equal'}</span></span>
            {config.verificationFee !== '0' && (
              <span style={{ color: '#6b6a8a' }}>Verification fee: <span style={{ color: '#ffaa00' }}>{formatAmount(config.verificationFee)} JUNOX</span></span>
            )}
